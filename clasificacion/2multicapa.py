...
class Detector:
	...
	def recortar(self):
		...
		recortes = []
		print "tamano"
		for i in range(0, len(contornosOrdenados)):
			x,y,w,h = cv2.boundingRect(contornosOrdenados[i]) # encuentra el recuadro
	        recorte = img[y:y+h, x:x+w] # recorte de la imagen
	        if len(recorte) > 0: # El recorte coniene algo
	        	#ancho = len(recorte[0])
	            #alto = len(recorte)
	            ...
	            recorte = cv2.resize(recorte, (width,height))
	            recortes.append(recorte.reshape(784))
	            print "Agregando recorte"
	            pass
...
#*********************************************************** Clasificador*******************++++++
start_time = time.time()
cantidadNeuronas = 10 # Cantidad de caracteres a reconocer
...
y_ = tf.placeholder(tf.float32, [None, cantidadNeuronas]) # Salidas ideales

# Pesos y Bias(no se sabe) Factor correccion
...
W1 = tf.Variable(tf.random_normal([n_hidden, cantidadNeuronas], stddev=0.01)) # Pesos psiapticos
B1 = tf.Variable(tf.random_normal([cantidadNeuronas], stddev=0.01))
...
xTest, yTest = datos.nextPaquete(10000)
...
for i in range(1,80):
	...
	#batchx = datos.entrenamiento['images']
	#batchy = datos.entrenamiento['labels']
	batchx, batchy = datos.nextPaquete(500) # Muestra estocastica de 1000 imagenes
	...
	loss1 = sess.run(mse, feed_dict = {x:batchx, y_:batchy} ) # Error sobre los datos de entrenamiento
	loss2 = sess.run(mse, feed_dict = {x:xTest, y_:yTest} ) # Error sobre los datos de evaluacion
...
print "tamano " + str(len(recortes))
for i in xrange(0,len(recortes)):
